...
class RunRegistry(Generic[S]):
    """Registry for one kind of run (``sync_run``, ``async_run``, ``validation``, …)."""
    ...
    def __init__(self, kind: str, model: type[S], store: RunStore | None = None):
        ...
        self._project: dict[str, str | None] = {}
    ...
    def create(self, state: S, project_id: str | None = None) -> None:
        """``project_id`` links the run to its project in the store."""
        run_id = state.run_id  # type: ignore[attr-defined]
        with self._lock:
            self._runs[run_id] = state
            self._flushed[run_id] = time.monotonic()
            self._project[run_id] = project_id or None
    ...
    def list(self, limit: int = 50, project_id: str | None = None) -> list[RunRecord]:
        try:
            return self.store.list(self._kind, limit, project_id)
