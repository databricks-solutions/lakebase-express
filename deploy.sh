...
#
# The app runs as its own service principal, so step 4 grants that SP access to
...
#
# Step 5 then checks the scope actually holds the Lakebase role password, which the
...
# Resolved config, reused below to render app.yaml and to read the app/scope names.
BUNDLE_JSON="$(db validate -o json)" || fail "Bundle validation failed — check databricks.yml and CLI auth (databricks auth login --host <workspace> --profile <profile>)."
...
# --- 2. Render app.yaml ------------------------------------------------------
# The Apps runtime reads app.yaml on every container start, while the bundle's
# `config` block applies at deploy time only — so generate one from the other.
echo "==> Rendering app.yaml from the bundle's resolved app config"
printf '%s' "$BUNDLE_JSON" | python3 -c '
import json, sys

cfg = json.load(sys.stdin)["resources"]["apps"]["lakebase_express"]["config"]
with open("app.yaml", "w") as f:
    f.write("# GENERATED by deploy.sh from databricks.yml + target.yml — do not edit.\n")
    # YAML is a superset of JSON, so every value dumps straight through as JSON.
    for key, value in cfg.items():
        f.write("%s: %s\n" % (key, json.dumps(value)))
env = cfg.get("env") or []
print("    %d env var(s)" % len(env))
blank = [e["name"] for e in env if "value" in e and not str(e["value"]).strip()]
if blank:
    print("    ⚠ empty value for: " + ", ".join(blank))
' || fail "Could not render app.yaml from the bundle config."

# --- 3. Deploy the bundle ----------------------------------------------------
echo "==> Deploying bundle (syncs source, creates/updates the app)"
db deploy || fail "Bundle deploy failed."

# --- 4. Grant the app's service principal access to the secret scope ---------
...
# --- 5. Check the scope holds the key the app needs --------------------------
